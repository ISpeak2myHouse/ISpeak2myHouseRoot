...
// delete voice + noise templates
#define DELETE_NOISE_TEMPLTATES false
// test with the noise templates 
#define NOISE_TEMPLATE true
/// the number of noise templates to be stored
#define NUMBER_OF_NOISES 2  
...
// _______________________________________________________
KeyWordDetector::KeyWordDetector(bool breadboard) {
...
bool KeyWordDetector::deleteAllTemplates() {
  bool success = true;
  int TEMPLATES_TO_KEEP;
  //Set the variable TEMPLATES_TO_KEEP to NOISE_TEMPLATES if noise templaes need not be deleted esle set it to 0 so all the templates get deleted
  if (DELETE_NOISE_TEMPLTATES){
    TEMPLATES_TO_KEEP = 0;
  }
    else{
      TEMPLATES_TO_KEEP = NUMBER_OF_NOISES ;
    }
  
   
  for (int i = TEMPLATES_TO_KEEP ; i < numberOfStoredTemplates; i++) {
...
int KeyWordDetector::testPotentialKeyword(size_t potentiallKeywordFrameCount) {
  ...
  for (int k = NUMBER_OF_NOISES; k < numberOfStoredTemplates; k++) {
    // If data was successfully read
    Serial.println("Comparing with the voice templates");
...
//----------------------------------- COMPARE THE NOISE STORED IN TEMPLAES 0-3 AND NOT TRIGGER IF MATCHED-----------------
 // This must be tested for all availables templates
  // These must be loaded from SD card sequentially since it is not enough memory left to store all
  // template files on mcu
 #ifdef NOISE_TEMPLATE
 Serial.println("Comparing with the noise templates");
  for (int k = 0; k < NUMBER_OF_NOISES; k++) {
    ...
    if (SD_readMFCCData(k)) {
      ...
      // Only if lenght matches, do the gifty stuff (DTW)
        ...
        if (meanDistance < MATCH_THRESHOLD) {
          if (COMPARE_ALL_KEYWORDS) {
            Serial.println("comparing all keywords");
            if (meanDistance < score) {
              score = meanDistance;
              keyword = -1;
            } 
