...
#define DEBUG true
#define MORE_DEBUG false
#define BREAD_BOARD true
...
#if BREAD_BOARD
KeyWordDetector keyWordDetector(true);
#else
KeyWordDetector keyWordDetector(false);
#endif
...
#if BREAD_BOARD
AudioInputAnalog         audioInput(A1);           //xy=105,63
#else
...
#if BREAD_BOARD
AudioOutputAnalog        audioOutput;           //xy=105,63
#else
...
void setup() {
  ...
  // Init the SD Card
  bool SDWorking = SD.begin(SDCARD_CS_PIN);
  if (SDWorking) {
    Serial.println(F("SD Card is working"));
  } else {
    Serial.println(F("SD Card is not working")); 
  }
...
void loop() {
  ...
  if (MORE_DEBUG) Serial.println(F("Looping"));
  
  // Updates the volume of the speaker if needed
  updateSpeakerVolume();

  if (MORE_DEBUG) Serial.println(F("Looping-updateSpeakerVolume"));
  // Recognize command
  handleCommandDetection();
  if (MORE_DEBUG) Serial.println(F("Looping-handleCommandDetection"));
  
  // Handle button presses
  handleButton();
  if (MORE_DEBUG) Serial.println(F("Looping-handleButton"));
  
  // Read data if available
  handleIncomingStream();
  if (MORE_DEBUG) Serial.println(F("Looping-handleIncomingStream"));
  
  // Stream data if needed
  handleOutgoingStream();
  if (MORE_DEBUG) Serial.println(F("Looping-handleOutgoingStream"));
}
...
//My Version of touch implementation :)
#define TOUCH_DETECT_THRSHLD 2000
int TouchRead(){

  
 //int TouchStatus=false; 
  if (!digitalRead(BUTTON_PIN) || touchRead(A5)>TOUCH_DETECT_THRSHLD) {
  return true;
  }
  else{
  return false;
...
void initAudioStream() {
  ...
  #if BREAD_BOARD
  delay (500);
  //The defualt pins are being used in this setup
  SPI.setMOSI(11);
  SPI.setMISO(12);
  SPI.setSCK(13);
  #else
  ...
  SPI.setSCK(14);
  #endif
  //Serial.println("Before setting NRF24");
  ...
  //audioStream.setPALevel(RF24_PA_MIN); 
...
void initAudioBoard() {
  ...
  #if BREAD_BOARD
  Serial.println("Skipping Audio Board Init");
  #else
  Serial.println("Audio Board Init");
